...
#
sed -e 's/^\([0-9]\)\//0\1\//g' -e 's/\/\([0-9]\)\//\/0\1\//g' -e 's/\/\([0-9][0-9]\);/\/20\1;/g' -e 's/^\([0-9][0-9]\)\/\([0-9][0-9]\)\/20\([0-9][0-9]\);/20\3-\2-\1;/g' -e 'y/abcn,;/ABCN.,/' -e 's/,N/,\\N/g' -e 's/,,/,\\N,/g' -e 's/,[^a-zA-Z0-9]$/,\\N/g' data.csv > output.csv